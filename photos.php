...
$size = 0;
$total_size = 0;
$database->query("SELECT DISTINCT name FROM files WHERE type = 'photo' and email = '$email' ORDER BY important DESC");
$row = $database->resultset();
$numrows = $database->rowCount();
foreach($row as $photo) {
   $photo = implode(' ', $photo);
   $size = $categories->getFileSize('upload/photos/'.$photo.'');
   $total_size = $size + $total_size;
   echo "<div class='container_full_width'><div class='container highlight'><div class='row'><div class='col-md-12 text-left padd'><a href='download_document?file=".$photo."'>".$photo."</div></div>";
   ...
   <a href='download_photo?file=".$photo."'><input type='button' name='submit' class='btn btn-success' value='Download' /></a> <input type='submit' name='submit' class='btn btn-primary' value='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Edit&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' /> <input type='submit' name='submit' class='btn btn-danger' value='&nbsp;&nbsp;&nbsp;Delete&nbsp;&nbsp;&nbsp;' />
   </form>
